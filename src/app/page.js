...
const Page = () => {
  ...
  return (<div className={`min-h-screen transition-all duration-500 ${darkMode ? 'bg-[#0f172a] text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
  ...
  <nav className="max-w-6xl mx-auto px-6 py-6 flex justify-between items-center">
    <div className="flex items-center gap-3">
      ...
      {/* SEO FIX: Using H1 for the main brand name is perfect */}
      <h1 className="text-xl font-bold tracking-tight cursor-default">LivePaste</h1>
    </div>
  ...
  <main className="max-w-5xl mx-auto px-6 py-4 grid lg:grid-cols-12 gap-8">
    ...
    <div className="lg:col-span-8">
      <section className={`relative rounded-3xl shadow-2xl border transition-all overflow-hidden ${
        ...
        <div className={`px-6 py-4 border-b flex justify-between items-center ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider opacity-70 cursor-default">
        ...
        {/* ACCESSIBILITY FIX: Added aria-label to textarea since there's no visible label */}
        <textarea 
          ...
          className={`w-full h-[500px] p-8 bg-transparent resize-none focus:outline-none text-lg font-mono leading-relaxed ${
            darkMode ? 'text-slate-200 placeholder-slate-700' : 'text-slate-800 placeholder-slate-400'
          }`}
    ...
    <div className="lg:col-span-4 space-y-6">
      <div className={`p-6 rounded-3xl border transition-all ${
        ...
        {/* SEMANTIC FIX: Changed H3 to H2 so the order is correct (H1 -> H2) */}
        {/* CONTRAST FIX: Changed text-blue-500 to text-blue-600 for better visibility */}
        <h2 className="text-sm font-bold uppercase tracking-widest text-blue-600 mb-6 flex items-center gap-2 cursor-default">
          <Globe size={16} /> Room Connection
        </h2>
        
        <div className="space-y-4">
          <div>
            {/* ACCESSIBILITY FIX: Added htmlFor to link label to input */}
            {/* CONTRAST FIX: Changed opacity-40 to text-slate-500 */}
            <label htmlFor="room-input" className="text-[10px] uppercase font-bold text-slate-500 mb-2 block ml-1">
              Easy Room Code
            </label>
          ...
          <button 
            aria-label={isJoined ? "Room is synced" : "Join room to share"}
        ...
        <div className={`mt-8 pt-6 border-t text-center ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}>
          {/* CONTRAST FIX: Removed opacity-50 and used text-slate-500 */}
          <p className="text-xs text-slate-500 italic cursor-default">
